...
export default function ScholarshipUpdateForm(props) {
  const {
    id: idProp,
    scholarship,
    onSuccess,
  ...
  const resetStateValues = () => {
    const cleanValues = scholarshipRecord
      ? { ...initialValues, ...scholarshipRecord }
      : initialValues;
    setScholarshipName(cleanValues.ScholarshipName);
    setExpirationDate(cleanValues.ExpirationDate);
    setOpeningDate(cleanValues.OpeningDate);
    setAmount(cleanValues.Amount);
    setApplicationLink(cleanValues.ApplicationLink);
    setCitizenshipStatus(cleanValues.CitizenshipStatus);
    setCollegeLevel(cleanValues.CollegeLevel);
    setDescription(cleanValues.Description);
    setEthnicity(cleanValues.Ethnicity);
    setInterests(cleanValues.Interests);
    setMaxGPA(cleanValues.maxGPA);
    setMinGPA(cleanValues.minGPA);
    setProvider(cleanValues.Provider);
    setStateOfResidency(cleanValues.StateOfResidency);
    setTypeOfProgram(cleanValues.TypeOfProgram);
    setErrors({});
  };
  const [scholarshipRecord, setScholarshipRecord] = React.useState(scholarship);
  React.useEffect(() => {
    const queryData = async () => {
      const record = idProp
        ? await DataStore.query(Scholarship, idProp)
        : scholarship;
      setScholarshipRecord(record);
    };
    queryData();
  }, [idProp, scholarship]);
  React.useEffect(resetStateValues, [scholarshipRecord]);
  const validations = {
  ...
  return (
    <Grid
      ...
      onSubmit={async (event) => {
        ...
        try {
          ...
          await DataStore.save(
            Scholarship.copyOf(scholarshipRecord, (updated) => {
      ...
      {...getOverrideProps(overrides, "ScholarshipUpdateForm")}
      {...rest}
    >
      <TextField
